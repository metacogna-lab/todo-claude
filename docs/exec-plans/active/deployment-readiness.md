...
## Phased Plan
### Phase A – Contract Enforcement 👩‍💻 *(in progress)*
- [ ] Replace remaining internal schemas (ExecutionResult, LinkGraph) with contract mirrors.
- [ ] Expose contract-validated `TraceResponse` via GraphQL & REST (if added) and add tests.
- [ ] Document contract version bump policy in `.agents/CONTRACTS.md`.

### Phase B – Observability & Evaluations
- [ ] Ensure every capture writes Langfuse spans; fail verification on missing spans/DevTools.
- [ ] Provide CLI/GraphQL commands to list evaluation snapshots and replay them.
- [ ] Dashboards: script to generate summary from `data/evals`, connect to monitoring.

### Phase C – Ops & Deployment
- [ ] Extend `doctor` command to validate all env requirements (`OPENAI_API_KEY`, connectors, Langfuse, Tavily, etc.) and surface contract mismatches.
- [ ] Add container image + deployment scripts (Dockerfile review, GitHub Actions release job) and document rollout steps.
- [ ] Harden CLI/GraphQL error handling, ensure structured error format per contracts.
